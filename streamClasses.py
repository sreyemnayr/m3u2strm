...
class Movie(object):
    ...
    def makeStream(self):
        ...
        moviedir = "/".join([media_path, typedir, directories[1]])
        if not os.path.exists("/".join([media_path, typedir])):
            os.mkdir("/".join([media_path, typedir]))
        if not os.path.exists(moviedir):
            os.mkdir(moviedir)
        tools.makeStrm("/".join([media_path, filename]), self.url)
...
class TVEpisode(object):
    ...
    def makeStream(self):
        ...
        directories = directories[:-1]
        typedir = directories[0]
        showdir = "/".join([media_path, typedir, directories[1]])
        if not os.path.exists("/".join([media_path, typedir])):
            os.mkdir("/".join([media_path, typedir]))
...
class rawStreamList(object):
    ...
    def readLines(self):
        self.lines = [
            line.decode(self.response.encoding or "utf-8")
            ...
            if thisline[0] == "#" and nextline[0] == "#":
                if tools.verifyURL(self.lines[linenumber + 2]):
                    ...
                    self.log.write_to_log(
                        msg=" ".join(
                            [
                                "Error finding raw stream in linenumber:",
                                str(linenumber),
                                "\n",
    ...
    def parseVodMovie(self, streaminfo, streamURL):
        ...
        if type(title) is not type(None) and title != "()":
            moviestream = Movie(
                title, streamURL, year=year, resolution=resolution, language=language
            )
