...
def tvgTypeMatch(line):
    ...
    if typematch:
        return typematch
    typematch_2 = re.compile('group-title="(.*?) VOD"', re.IGNORECASE).search(line)
    if typematch_2:
        return typematch_2
    return
...
def ufcwweMatch(line):
    ufcwwematch = re.compile(
        "[U][f][c]|[w][w][e]|[r][i][d][i][c][u][l]", re.IGNORECASE
    ).search(line)
    if ufcwwematch:
        return ufcwwematch
    return
...
def episodeMatch(line):
    episodematch = re.compile(
        "[e][0-9][0-9]|[0-9][0-9][x][0-9][0-9]", re.IGNORECASE
    ).search(line)
    if episodematch:
...
def seasonMatch(line):
    seasonmatch = re.compile(
        "[s][0-9][0-9]|[0-9][0-9][x][0-9][0-9]", re.IGNORECASE
    ).search(line)
    if seasonmatch:
